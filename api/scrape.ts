...
const UA = 'Mozilla/5.0 (Linux; Android 10; Pixel 3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123 Mobile Safari/537.36';
...
async function parseResultsHTML(html:string){
  ...
  $('a.fr-link[data-test="searchResult-link"]').each((_,a)=>{
    ...
    if(href)links.push(href.startsWith('http')?href:\`\${BASE}\${href}\`);
  });
  const hasNext=$('button#btn-next20, button[data-test="pagerSearchAnnuaire"], a[rel="next"]').length>0;
...
async function parseDetail(url:string){
  ...
  $('a[href^="http"]').each((_,a)=>{
    const href=$(a).attr('href')!;
    if(!href.includes('service-public.gouv.fr')){site=href;return false;}
  });
...
async function crawlWithPuppeteer(what:string,where?:string,maxClicks=15){
  ...
  try{
    ...
    const collect=async()=>{
      const items=await page.$$eval('a.fr-link[data-test="searchResult-link"]',as=>as.map(a=>(a as HTMLAnchorElement).href));
...
export default async function handler(req:VercelRequest,res:VercelResponse){
  try{
    ...
    let links:string[]=[];
    try{links=await crawlCheerio(what,where);}catch{}
    if(!links.length)links=await crawlWithPuppeteer(what,where);
    const rows:any[]=[];
    for(const u of links){
      try{rows.push(await parseDetail(u));await sleep(500);}catch{}
    }
    if(format==='xlsx'){
  ...
  }catch(e:any){res.status(500).json({error:e?.message||'Erreur serveur'});}
}
